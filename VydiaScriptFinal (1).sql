...
-- ============================================
-- População de GRADES por N1 e N2
-- grade_type: '1' = N1, '2' = N2
-- ============================================

-- N1
INSERT INTO Grades (fk_student, fk_subject, grade_type, value, send_at)
SELECT
  s.id_student,
  ta.fk_subject,
  '1'::char(1) AS grade_type,   -- N1
  ROUND((6 + random() * 4)::numeric, 2) AS value,  -- [6.00, 10.00]
  NOW() - (INTERVAL '20 days' * (random() * 1.0))
FROM Students s
JOIN TeachingAssignment ta
  ON ta.fk_class = s.fk_class
WHERE NOT EXISTS (
  SELECT 1
  FROM Grades g
  WHERE g.fk_student = s.id_student
    AND g.fk_subject = ta.fk_subject
    AND g.grade_type = '1'
);
...
SELECT
  s.id_student,
  ta.fk_subject,
  '2'::char(1) AS grade_type,   -- N2
  ROUND((6 + random() * 4)::numeric, 2) AS value,  -- [6.00, 10.00]
  NOW() - (INTERVAL '10 days' * (random() * 1.0))
FROM Students s
JOIN TeachingAssignment ta
  ON ta.fk_class = s.fk_class
WHERE NOT EXISTS (
...
-- 11) receive (quem recebeu qual relatório)
